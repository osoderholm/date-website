...
def index(request):
    ...
    return render(request, '../templates/news/index.html', {'latest_news_items': page_obj})
    

def article(request, slug):
    post = models.Post.objects.get(slug=slug, published=True, albins_angels=False)
    return render(request, '../templates/news/article.html', {'article': post})


def author(request, author):
    articles = models.Post.objects.filter(author__username__exact=author, published=True).order_by('modified_time')
    return render(request, '../templates/news/author.html', {'articles': articles})
